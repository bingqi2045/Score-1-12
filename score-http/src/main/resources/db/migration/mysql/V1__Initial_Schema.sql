-- ----------------------------------------------------
-- Migration script for Score v2.4.0 Initialization  --
--                                                   --
...
CREATE TABLE `abie`
(
    `abie_id`                   char(36) CHARACTER SET ascii    NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                      char(32) CHARACTER SET ascii    NOT NULL COMMENT 'A globally unique identifier (GUID).',
    `based_acc_manifest_id`     char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key to the ACC_MANIFEST table refering to the ACC, on which the business context has been applied to derive this ABIE.',
    ...
    `biz_ctx_id`                char(36) CHARACTER SET ascii DEFAULT NULL COMMENT '(Deprecated) A foreign key to the BIZ_CTX table. This column stores the business context assigned to the ABIE.',
    ...
    `created_by`                char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who creates the ABIE. The creator of the ABIE is also its owner by default. ABIEs created as children of another ABIE have the same CREATED_BY as its parent.',
    `last_updated_by`           char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the last user who has updated the ABIE record. This may be the user who is in the same group as the creator.',
    ...
    `owner_top_level_asbiep_id` char(36) CHARACTER SET ascii    NOT NULL COMMENT 'This is a foreign key to the top-level ASBIEP.',
    PRIMARY KEY (`abie_id`),
...
CREATE TABLE `acc`
(
    `acc_id`                 char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                   char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `based_acc_id`           char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'BASED_ACC_ID is a foreign key to the ACC table itself. It represents the ACC that is qualified by this ACC. In general CCS sense, a qualification can be a content extension or restriction, but the current scope supports only extension.',
    ...
    `namespace_id`           char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the NAMESPACE table.',
    `created_by`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the user who creates the entity.\n\nThis column never change between the history and the current record for a given revision. The history record should have the same value as that of its current record.',
    `owner_user_id`          char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `last_updated_by`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record. \n\nIn the history record, this should always be the user who is editing the entity (perhaps except when the ownership has just been changed).',
    ...
    `replacement_acc_id`     char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    ...
    `prev_acc_id`            char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_acc_id`            char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
    PRIMARY KEY (`acc_id`),
...
(
    `acc_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                  char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `acc_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `based_acc_manifest_id`       char(36) CHARACTER SET ascii DEFAULT NULL,
    `conflict`                    tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `log_id`                      char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A foreign key pointed to a log for the current record.',
    `replacement_acc_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_acc_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_acc_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                           `acc_manifest_replacement_acc_manifest_id_fk` (`replacement_acc_manifest_id`),
    ...
    CONSTRAINT `acc_manifest_replacement_acc_manifest_id_fk` FOREIGN KEY (`replacement_acc_manifest_id`) REFERENCES `acc_manifest` (`acc_manifest_id`)
...
(
    `acc_manifest_id` char(36) CHARACTER SET ascii NOT NULL,
    `cc_tag_id`       char(36) CHARACTER SET ascii NOT NULL,
...
(
    `agency_id_list_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                          char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `agency_id_list_value_id`       char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is the identification of the agency or organization which developed and/or maintains the list. Theoretically, this can be modeled as a self-reference foreign key, but it is not implemented at this point.',
    `version_id`                    varchar(100)                 DEFAULT NULL COMMENT 'Version number of the agency identification list (assigned by the agency).',
    `based_agency_id_list_id`       char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is a foreign key to the AGENCY_ID_LIST table itself. This identifies the agency id list on which this agency id list is based, if any. The derivation may be restriction and/or extension.',
    ...
    `namespace_id`                  char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the NAMESPACE table.',
    `created_by`                    char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created the agency ID list.',
    `last_updated_by`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It identifies the user who last updated the agency ID list.',
    ...
    `replacement_agency_id_list_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    `owner_user_id`                 char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `prev_agency_id_list_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_agency_id_list_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
...
(
    `agency_id_list_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `agency_id_list_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `agency_id_list_value_manifest_id`       char(36) CHARACTER SET ascii DEFAULT NULL,
    `based_agency_id_list_manifest_id`       char(36) CHARACTER SET ascii DEFAULT NULL,
    `conflict`                               tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `log_id`                                 char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A foreign key pointed to a log for the current record.',
    `replacement_agency_id_list_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_agency_id_list_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_agency_id_list_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                                      `agency_id_list_manifest_agency_id_list_value_manifest_id_fk` (`agency_id_list_value_manifest_id`),
    KEY                                      `agency_id_list_manifest_based_agency_id_list_manifest_id_fk` (`based_agency_id_list_manifest_id`),
    KEY                                      `agency_id_list_manifest_replacement_agency_id_list_manifest_i_fk` (`replacement_agency_id_list_manifest_id`),
    ...
    CONSTRAINT `agency_id_list_manifest_agency_id_list_value_manifest_id_fk` FOREIGN KEY (`agency_id_list_value_manifest_id`) REFERENCES `agency_id_list_value_manifest` (`agency_id_list_value_manifest_id`),
    ...
    CONSTRAINT `agency_id_list_manifest_replacement_agency_id_list_manifest_i_fk` FOREIGN KEY (`replacement_agency_id_list_manifest_id`) REFERENCES `agency_id_list_manifest` (`agency_id_list_manifest_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
    `agency_id_list_value_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                                char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `owner_list_id`                       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the agency identification list in the AGENCY_ID_LIST table this value belongs to.',
    `based_agency_id_list_value_id`       char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the AGENCY_ID_LIST_VALUE table itself. This column is used when the AGENCY_ID_LIST_VALUE is derived from the based AGENCY_ID_LIST_VALUE.',
    ...
    `replacement_agency_id_list_value_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    `created_by`                          char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created the agency ID list value.',
    `owner_user_id`                       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `last_updated_by`                     char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It identifies the user who last updated the agency ID list value.',
    ...
    `prev_agency_id_list_value_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_agency_id_list_value_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
...
(
    `agency_id_list_value_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                                   char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `agency_id_list_value_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `agency_id_list_manifest_id`                   char(36) CHARACTER SET ascii NOT NULL,
    `based_agency_id_list_value_manifest_id`       char(36) CHARACTER SET ascii DEFAULT NULL,
    `conflict`                                     tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `replacement_agency_id_list_value_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_agency_id_list_value_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_agency_id_list_value_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                                            `agency_id_list_value_manifest_based_agency_id_list_value_mani_fk` (`based_agency_id_list_value_manifest_id`),
    KEY                                            `agency_id_list_value_manifest_replacement_agency_id_list_valu_fk` (`replacement_agency_id_list_value_manifest_id`),
    ...
    CONSTRAINT `agency_id_list_value_manifest_based_agency_id_list_value_mani_fk` FOREIGN KEY (`based_agency_id_list_value_manifest_id`) REFERENCES `agency_id_list_value_manifest` (`agency_id_list_value_manifest_id`),
    ...
    CONSTRAINT `agency_id_list_value_manifest_replacement_agency_id_list_valu_fk` FOREIGN KEY (`replacement_agency_id_list_value_manifest_id`) REFERENCES `agency_id_list_value_manifest` (`agency_id_list_value_manifest_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
    `app_oauth2_user_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `app_user_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A reference to the record in `app_user`. If it is not set, this is treated as a pending record.',
    `oauth2_app_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'A reference to the record in `oauth2_app`.',
    ...
    UNIQUE KEY `app_oauth2_user_uk1` (`sub`),
    KEY                  `app_oauth2_user_app_user_id_fk` (`app_user_id`),
    KEY                  `app_oauth2_user_oauth2_app_id_fk` (`oauth2_app_id`),
...
(
    `app_user_id`  char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
...
(
    ...
    `remark`                    varchar(225)                 DEFAULT NULL COMMENT 'This column allows the user to specify very context-specific usage of the BIE. It is different from the DEFINITION column in that the DEFINITION column is a description conveying the meaning of the associated concept. Remarks may be a very implementation specific instruction or others. For example, BOM BOD, as an ACC, is a generic BOM structure. In a particular context, a BOM ABIE can be a Super BOM. Explanation of the Super BOM concept should be captured in the Definition of the ABIE. A remark about that ABIE may be "Type of BOM should be recognized in the BOM/typeCode."',
    `created_by`                char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who creates the ASBIE. The creator of the ASBIE is also its owner by default. ASBIEs created as children of another ABIE have the same CREATED_BY.',
    `last_updated_by`           char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who has last updated the ASBIE record.',
...
(
    `asbie_bizterm_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `ascc_bizterm_id`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'An internal ID of the ascc_business_term record.',
    `asbie_id`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'An internal ID of the associated ASBIE',
    ...
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the user who creates the ASBIE_BIZTERM record. The creator of the ASBIE_BIZTERM is also its owner by default.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the last user who has updated the ASBIE_BIZTERM record. This may be the user who is in the same group as the creator.',
    ...
    KEY                     `asbie_bizterm_created_by_fk` (`created_by`),
    KEY                     `asbie_bizterm_last_updated_by_fk` (`last_updated_by`),
    KEY                     `asbie_bizterm_asbie_id_fk` (`asbie_id`),
    KEY                     `asbie_bizterm_ascc_bizterm_id_fk` (`ascc_bizterm_id`),
    CONSTRAINT `asbie_bizterm_asbie_id_fk` FOREIGN KEY (`asbie_id`) REFERENCES `asbie` (`asbie_id`),
    CONSTRAINT `asbie_bizterm_ascc_bizterm_id_fk` FOREIGN KEY (`ascc_bizterm_id`) REFERENCES `ascc_bizterm` (`ascc_bizterm_id`),
    CONSTRAINT `asbie_bizterm_created_by_fk` FOREIGN KEY (`created_by`) REFERENCES `app_user` (`app_user_id`),
    CONSTRAINT `asbie_bizterm_last_updated_by_fk` FOREIGN KEY (`last_updated_by`) REFERENCES `app_user` (`app_user_id`)
...
CREATE TABLE `asbiep`
(
    `asbiep_id`                 char(36) CHARACTER SET ascii    NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                      char(32) CHARACTER SET ascii    NOT NULL COMMENT 'A globally unique identifier (GUID).',
    `based_asccp_manifest_id`   char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key pointing to the ASCCP_MANIFEST record. It is the ASCCP, on which the ASBIEP contextualizes.',
    `path`                      text CHARACTER SET ascii,
    `hash_path`                 varchar(64) CHARACTER SET ascii NOT NULL COMMENT 'hash_path generated from the path of the component graph using hash function, so that it is unique in the graph.',
    `role_of_abie_id`           char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key pointing to the ABIE record. It is the ABIE, which the property term in the based ASCCP qualifies. Note that the ABIE has to be derived from the ACC used by the based ASCCP.',
    ...
    `created_by`                char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who creates the ASBIEP. The creator of the ASBIEP is also its owner by default. ASBIEPs created as children of another ABIE have the same CREATED_BY.',
    `last_updated_by`           char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the last user who has updated the ASBIEP record.',
...
CREATE TABLE `ascc`
(
    `ascc_id`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `from_acc_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'FROM_ACC_ID is a foreign key pointing to an ACC record. It is basically pointing to a parent data element (type) of the TO_ASCCP_ID.',
    `to_asccp_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'TO_ASCCP_ID is a foreign key to an ASCCP table record. It is basically pointing to a child data element of the FROM_ACC_ID.',
    ...
    `replacement_ascc_id`   char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key to the APP_USER table referring to the user who creates the entity.\n\nThis column never change between the history and the current record for a given revision. The history record should have the same value as that of its current record.',
    `owner_user_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key to the APP_USER table referring to the last user who has updated the record. \n\nIn the history record, this should always be the user who is editing the entity (perhaps except when the ownership has just been changed).',
    ...
    `prev_ascc_id`          char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_ascc_id`          char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
    PRIMARY KEY (`ascc_id`),
...
(
    `ascc_bizterm_id`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `business_term_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'An internal ID of the associated business term.',
    `ascc_id`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'An internal ID of the associated ASCC',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the user who creates the ASCC_BIZTERM record. The creator of the ASCC_BIZTERM is also its owner by default.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the last user who has updated the ASCC_BIZTERM record. This may be the user who is in the same group as the creator.',
    ...
    KEY                     `ascc_bizterm_created_by_fk` (`created_by`),
    KEY                     `ascc_bizterm_last_updated_by_fk` (`last_updated_by`),
    KEY                     `ascc_bizterm_ascc_id_fk` (`ascc_id`),
    KEY                     `ascc_bizterm_business_term_id_fk` (`business_term_id`),
    CONSTRAINT `ascc_bizterm_ascc_id_fk` FOREIGN KEY (`ascc_id`) REFERENCES `ascc` (`ascc_id`),
    CONSTRAINT `ascc_bizterm_business_term_id_fk` FOREIGN KEY (`business_term_id`) REFERENCES `business_term` (`business_term_id`),
    CONSTRAINT `ascc_bizterm_created_by_fk` FOREIGN KEY (`created_by`) REFERENCES `app_user` (`app_user_id`),
    CONSTRAINT `ascc_bizterm_last_updated_by_fk` FOREIGN KEY (`last_updated_by`) REFERENCES `app_user` (`app_user_id`)
...
(
    `ascc_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                   char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the RELEASE table.',
    `ascc_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `seq_key_id`                   char(36) CHARACTER SET ascii DEFAULT NULL,
    `from_acc_manifest_id`         char(36) CHARACTER SET ascii NOT NULL,
    `to_asccp_manifest_id`         char(36) CHARACTER SET ascii NOT NULL,
    `conflict`                     tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `replacement_ascc_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_ascc_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_ascc_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                            `ascc_manifest_replacement_ascc_manifest_id_fk` (`replacement_ascc_manifest_id`),
    ...
    CONSTRAINT `ascc_manifest_replacement_ascc_manifest_id_fk` FOREIGN KEY (`replacement_ascc_manifest_id`) REFERENCES `ascc_manifest` (`ascc_manifest_id`),
...
CREATE TABLE `asccp`
(
    `asccp_id`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `definition_source`     varchar(100)                 DEFAULT NULL COMMENT 'This is typically a URL identifying the source of the DEFINITION column.',
    `role_of_acc_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'The ACC from which this ASCCP is created (ASCCP applies role to the ACC).',
    `den`                   varchar(200)                 DEFAULT NULL COMMENT 'The dictionary entry name of the ASCCP.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the user who creates the entity. \n\nThis column never change between the history and the current record for a given revision. The history record should have the same value as that of its current record.',
    `owner_user_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who has updated the record. \n\nIn the history record, this should always be the user who is editing the entity (perhaps except when the ownership has just been changed).',
    ...
    `replacement_asccp_id`  char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    ...
    `prev_asccp_id`         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_asccp_id`         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
...
(
    `asccp_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                    char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `asccp_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `role_of_acc_manifest_id`       char(36) CHARACTER SET ascii NOT NULL,
    `conflict`                      tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `log_id`                        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A foreign key pointed to a log for the current record.',
    `replacement_asccp_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_asccp_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_asccp_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                             `asccp_manifest_replacement_asccp_manifest_id_fk` (`replacement_asccp_manifest_id`),
    ...
    CONSTRAINT `asccp_manifest_replacement_asccp_manifest_id_fk` FOREIGN KEY (`replacement_asccp_manifest_id`) REFERENCES `asccp_manifest` (`asccp_manifest_id`),
...
(
    `asccp_manifest_id` char(36) CHARACTER SET ascii NOT NULL,
    `cc_tag_id`         char(36) CHARACTER SET ascii NOT NULL,
...
CREATE TABLE `bbie`
(
    `bbie_id`                   char(36) CHARACTER SET ascii    NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                      char(32) CHARACTER SET ascii    NOT NULL COMMENT 'A globally unique identifier (GUID).',
    `based_bcc_manifest_id`     char(36) CHARACTER SET ascii    NOT NULL COMMENT 'The BASED_BCC_MANIFEST_ID column refers to the BCC_MANIFEST record, which this BBIE contextualizes.',
    `path`                      text CHARACTER SET ascii,
    `hash_path`                 varchar(64) CHARACTER SET ascii NOT NULL COMMENT 'hash_path generated from the path of the component graph using hash function, so that it is unique in the graph.',
    ...
    `to_bbiep_id`               char(36) CHARACTER SET ascii    NOT NULL COMMENT 'TO_BBIEP_ID is a foreign key to the BBIEP table. TO_BBIEP_ID basically refers to a child data element of the FROM_ABIE_ID. TO_BBIEP_ID must be based on the TO_BCCP_ID in the based BCC.',
    `bdt_pri_restri_id`         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is the foreign key to the BDT_PRI_RESTRI table. It indicates the primitive assigned to the BBIE (or also can be viewed as assigned to the BBIEP for this specific association). This is assigned by the user who authors the BIE. The assignment would override the default from the CC side.',
    `code_list_id`              char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is a foreign key to the CODE_LIST table. If a code list is assigned to the BBIE (or also can be viewed as assigned to the BBIEP for this association), then this column stores the assigned code list. It should be noted that one of the possible primitives assignable to the BDT_PRI_RESTRI_ID column may also be a code list. So this column is typically used when the user wants to assign another code list different from the one permissible by the CC model.',
    `agency_id_list_id`         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is a foreign key to the AGENCY_ID_LIST table. It is used in the case that the BDT content can be restricted to an agency identification.',
    ...
    `example`                   text,
    `remark`                    varchar(225)                 DEFAULT NULL COMMENT 'This column allows the user to specify very context-specific usage of the BIE. It is different from the DEFINITION column in that the DEFINITION column is a description conveying the meaning of the associated concept. Remarks may be a very implementation specific instruction or others. For example, BOM BOD, as an ACC, is a generic BOM structure. In a particular context, a BOM ABIE can be a Super BOM. Explanation of the Super BOM concept should be captured in the Definition of the ABIE. A remark about that ABIE may be "Type of BOM should be recognized in the BOM/typeCode."',
    `created_by`                char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who creates the BBIE. The creator of the BBIE is also its owner by default. BBIEs created as children of another ABIE have the same CREATED_BY.',
    `last_updated_by`           char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who has last updated the BBIE record.',
...
(
    `bbie_bizterm_id`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `bcc_bizterm_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'An internal ID of the bcc_business_term record.',
    `bbie_id`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'An internal ID of the associated BBIE',
    `primary_indicator`     char(1)  DEFAULT NULL COMMENT 'The indicator shows if the business term is primary for the assigned BBIE.',
    `type_code`             char(30) DEFAULT NULL COMMENT 'The type code of the assignment.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the user who creates the BBIE_BIZTERM record. The creator of the ASBIE_BIZTERM is also its owner by default.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the last user who has updated the BBIE_BIZTERM record. This may be the user who is in the same group as the creator.',
    ...
    KEY                     `bbie_bizterm_created_by_fk` (`created_by`),
    KEY                     `bbie_bizterm_last_updated_by_fk` (`last_updated_by`),
    KEY                     `bbie_bizterm_bbie_id_fk` (`bbie_id`),
    KEY                     `bbie_bizterm_bcc_bizterm_id_fk` (`bcc_bizterm_id`),
    CONSTRAINT `bbie_bizterm_bbie_id_fk` FOREIGN KEY (`bbie_id`) REFERENCES `bbie` (`bbie_id`),
    CONSTRAINT `bbie_bizterm_bcc_bizterm_id_fk` FOREIGN KEY (`bcc_bizterm_id`) REFERENCES `bcc_bizterm` (`bcc_bizterm_id`),
    CONSTRAINT `bbie_bizterm_created_by_fk` FOREIGN KEY (`created_by`) REFERENCES `app_user` (`app_user_id`),
    CONSTRAINT `bbie_bizterm_last_updated_by_fk` FOREIGN KEY (`last_updated_by`) REFERENCES `app_user` (`app_user_id`)
...
(
    `bbie_sc_id`                char(36) CHARACTER SET ascii    NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                      char(32) CHARACTER SET ascii    NOT NULL COMMENT 'A globally unique identifier (GUID).',
    `based_dt_sc_manifest_id`   char(36) CHARACTER SET ascii    NOT NULL COMMENT 'Foreign key to the DT_SC_MANIFEST table. This should correspond to the DT_SC of the BDT of the based BCC and BCCP.',
    `path`                      text CHARACTER SET ascii,
    `hash_path`                 varchar(64) CHARACTER SET ascii NOT NULL COMMENT 'hash_path generated from the path of the component graph using hash function, so that it is unique in the graph.',
    `bbie_id`                   char(36) CHARACTER SET ascii    NOT NULL COMMENT 'The BBIE this BBIE_SC applies to.',
    `dt_sc_pri_restri_id`       char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This must be one of the allowed primitive/code list as specified in the corresponding SC of the based BCC of the BBIE (referred to by the BBIE_ID column).\n\nIt is the foreign key to the BDT_SC_PRI_RESTRI table. It indicates the primitive assigned to the BBIE (or also can be viewed as assigned to the BBIEP for this specific association). This is assigned by the user who authors the BIE. The assignment would override the default from the CC side.\n\nThis column, the CODE_LIST_ID column, and AGENCY_ID_LIST_ID column cannot have a value at the same time.',
    `code_list_id`              char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is a foreign key to the CODE_LIST table. If a code list is assigned to the BBIE SC (or also can be viewed as assigned to the BBIEP SC for this association), then this column stores the assigned code list. It should be noted that one of the possible primitives assignable to the DT_SC_PRI_RESTRI_ID column may also be a code list. So this column is typically used when the user wants to assign another code list different from the one permissible by the CC model.\n\nThis column is, the DT_SC_PRI_RESTRI_ID column, and AGENCY_ID_LIST_ID column cannot have a value at the same time.',
    ...
    `created_by`                char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who creates the BBIE_SC. The creator of the BBIE_SC is also its owner by default.',
    `last_updated_by`           char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who has last updated the BBIE_SC record.',
...
CREATE TABLE `bbiep`
(
    `bbiep_id`                  char(36) CHARACTER SET ascii    NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                      char(32) CHARACTER SET ascii    NOT NULL COMMENT 'A globally unique identifier (GUID).',
    `based_bccp_manifest_id`    char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key pointing to the BCCP_MANIFEST record. It is the BCCP, on which the BBIEP contextualizes.',
    `path`                      text CHARACTER SET ascii,
    `hash_path`                 varchar(64) CHARACTER SET ascii NOT NULL COMMENT 'hash_path generated from the path of the component graph using hash function, so that it is unique in the graph.',
    ...
    `created_by`                char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the user who creates the BBIEP. The creator of the BBIEP is also its owner by default. BBIEPs created as children of another ABIE have the same CREATED_BY'',',
    `last_updated_by`           char(36) CHARACTER SET ascii    NOT NULL COMMENT 'A foreign key referring to the last user who has updated the BBIEP record.',
...
CREATE TABLE `bcc`
(
    `bcc_id`                char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `to_bccp_id`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'TO_BCCP_ID is a foreign key to an BCCP table record. It is basically pointing to a child data element of the FROM_ACC_ID. \n\nNote that for the BCC history records, this column always points to the BCCP_ID of the current record of a BCCP.',
    `from_acc_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'FROM_ACC_ID is a foreign key pointing to an ACC record. It is basically pointing to a parent data element (type) of the TO_BCCP_ID. \n\nNote that for the BCC history records, this column always points to the ACC_ID of the current record of an ACC.',
    `seq_key`               int(11) DEFAULT NULL COMMENT '@deprecated since 2.0.0. This indicates the order of the associations among other siblings. A valid value is positive integer. The SEQ_KEY at the CC side is localized. In other words, if an ACC is based on another ACC, SEQ_KEY of ASCCs or BCCs of the former ACC starts at 1 again.',
    ...
    `state`                 varchar(20)                  DEFAULT NULL COMMENT 'Deleted, WIP, Draft, QA, Candidate, Production, Release Draft, Published. This the revision life cycle state of the BCC.\n\nState change can''t be undone. But the history record can still keep the records of when the state was changed.',
    `is_deprecated`         tinyint(1) NOT NULL COMMENT 'Indicates whether the CC is deprecated and should not be reused (i.e., no new reference to this record should be created).',
    `replacement_bcc_id`    char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    ...
    `fixed_value`           text COMMENT 'This column captures the fixed value constraint. Default and fixed value constraints cannot be used at the same time.',
    `prev_bcc_id`           char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_bcc_id`           char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
    PRIMARY KEY (`bcc_id`),
...
(
    `bcc_bizterm_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `business_term_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'An internal ID of the associated business term.',
    `bcc_id`                char(36) CHARACTER SET ascii NOT NULL COMMENT 'An internal ID of the associated BCC',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the user who creates the BCC_BIZTERM record. The creator of the BCC_BIZTERM is also its owner by default.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the last user who has updated the BCC_BIZTERM record. This may be the user who is in the same group as the creator.',
    ...
    KEY                     `bcc_bizterm_created_by_fk` (`created_by`),
    KEY                     `bcc_bizterm_last_updated_by_fk` (`last_updated_by`),
    KEY                     `bcc_bizterm_bcc_id_fk` (`bcc_id`),
    KEY                     `bcc_bizterm_business_term_id_fk` (`business_term_id`),
    CONSTRAINT `bcc_bizterm_bcc_id_fk` FOREIGN KEY (`bcc_id`) REFERENCES `bcc` (`bcc_id`),
    CONSTRAINT `bcc_bizterm_business_term_id_fk` FOREIGN KEY (`business_term_id`) REFERENCES `business_term` (`business_term_id`),
    CONSTRAINT `bcc_bizterm_created_by_fk` FOREIGN KEY (`created_by`) REFERENCES `app_user` (`app_user_id`),
    CONSTRAINT `bcc_bizterm_last_updated_by_fk` FOREIGN KEY (`last_updated_by`) REFERENCES `app_user` (`app_user_id`)
...
(
    `bcc_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                  char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the RELEASE table.',
    `bcc_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `seq_key_id`                  char(36) CHARACTER SET ascii DEFAULT NULL,
    `from_acc_manifest_id`        char(36) CHARACTER SET ascii NOT NULL,
    `to_bccp_manifest_id`         char(36) CHARACTER SET ascii NOT NULL,
    `conflict`                    tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `replacement_bcc_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_bcc_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_bcc_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                           `bcc_manifest_replacement_bcc_manifest_id_fk` (`replacement_bcc_manifest_id`),
    ...
    CONSTRAINT `bcc_manifest_replacement_bcc_manifest_id_fk` FOREIGN KEY (`replacement_bcc_manifest_id`) REFERENCES `bcc_manifest` (`bcc_manifest_id`),
...
CREATE TABLE `bccp`
(
    `bccp_id`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `bdt_id`                char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key pointing to the DT table indicating the data typye or data format of the BCCP. Only DT_ID which DT_Type is BDT can be used.',
    ...
    `namespace_id`          char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the NAMESPACE table.',
    `is_deprecated`         tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Indicates whether the CC is deprecated and should not be reused (i.e., no new reference to this record should be created).',
    `replacement_bccp_id`   char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the user who creates the entity.\n\nThis column never change between the history and the current record. The history record should have the same value as that of its current record.',
    `owner_user_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who has updated the record. \n\nIn the history record, this should always be the user who is editing the entity (perhaps except when the ownership has just been changed).',
    ...
    `default_value`         text COMMENT 'This column specifies the default value constraint. Default and fixed value constraints cannot be used at the same time.',
    `fixed_value`           text COMMENT 'This column captures the fixed value constraint. Default and fixed value constraints cannot be used at the same time.',
    `prev_bccp_id`          char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_bccp_id`          char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
    PRIMARY KEY (`bccp_id`),
...
(
    `bccp_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                   char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `bccp_id`                      char(36) CHARACTER SET ascii DEFAULT NULL,
    `bdt_manifest_id`              char(36) CHARACTER SET ascii NOT NULL,
    `conflict`                     tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `log_id`                       char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A foreign key pointed to a log for the current record.',
    `replacement_bccp_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_bccp_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_bccp_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                            `bccp_manifest_replacement_bccp_manifest_id_fk` (`replacement_bccp_manifest_id`),
    ...
    CONSTRAINT `bccp_manifest_replacement_bccp_manifest_id_fk` FOREIGN KEY (`replacement_bccp_manifest_id`) REFERENCES `bccp_manifest` (`bccp_manifest_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
    `bccp_manifest_id` char(36) CHARACTER SET ascii NOT NULL,
    `cc_tag_id`        char(36) CHARACTER SET ascii NOT NULL,
...
(
    `bdt_pri_restri_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `bdt_id`                      char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the DT table. It shall point to only DT that is a BDT (not a CDT).',
    `cdt_awd_pri_xps_type_map_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the CDT_AWD_PRI_XPS_TYPE_MAP table.',
    `code_list_id`                char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the CODE_LIST table.',
    `agency_id_list_id`           char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is a foreign key to the AGENCY_ID_LIST table. It is used in the case that the BDT content can be restricted to an agency identification.',
...
(
    `bdt_sc_pri_restri_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `bdt_sc_id`                      char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the DT table. It shall point to only DT that is a BDT (not a CDT).',
    `cdt_sc_awd_pri_xps_type_map_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the CDT_SC_AWD_PRI_XPS_TYPE_MAP table.',
    `code_list_id`                   char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the CODE_LIST table.',
    `agency_id_list_id`              char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is a foreign key to the AGENCY_ID_LIST table. It is used in the case that the BDT content can be restricted to an agency identification.',
...
(
    `bie_usage_rule_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `assigned_usage_rule_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the USAGE_RULE table indicating the usage rule assigned to a BIE.',
    `target_abie_id`         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the ABIE table indicating the ABIE, to which the usage rule is applied.',
    `target_asbie_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the ASBIE table indicating the ASBIE, to which the usage rule is applied.',
    `target_asbiep_id`       char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the ASBIEP table indicating the ASBIEP, to which the usage rule is applied.',
    `target_bbie_id`         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the BBIE table indicating the BBIE, to which the usage rule is applied.',
    `target_bbiep_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the BBIEP table indicating the ABIEP, to which the usage rule is applied.',
...
(
    `bie_user_ext_revision_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    ...
    `user_ext_acc_id`          char(36) CHARACTER SET ascii NOT NULL COMMENT 'This column points to the specific revision of a User Extension ACC (this is an ACC whose OAGIS_COMPONENT_TYPE = 4) currently used by the ABIE as indicated by the EXT_ABIE_ID or the by the TOP_LEVEL_ABIE_ID (in case of the AllExtension).',
    ...
    `top_level_asbiep_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'This is a foreign key to the top-level ASBIEP.',
...
(
    `biz_ctx_id`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    `name`                  varchar(100) DEFAULT NULL COMMENT 'Short, descriptive name of the business context.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the user who creates the entity.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table  referring to the last user who has updated the business context.',
...
(
    `biz_ctx_assignment_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `biz_ctx_id`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the biz_ctx table.',
    `top_level_asbiep_id`   char(36) CHARACTER SET ascii NOT NULL COMMENT 'This is a foreign key to the top-level ASBIEP.',
...
(
    `biz_ctx_value_id`    char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `biz_ctx_id`          char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the biz_ctx table.',
    `ctx_scheme_value_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the CTX_SCHEME_VALUE table.',
...
(
    `blob_content_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
...
(
    `blob_content_manifest_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `blob_content_id`               char(36) CHARACTER SET ascii NOT NULL,
    `release_id`                    char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `conflict`                      tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `prev_blob_content_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_blob_content_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL,
...
(
    `business_term_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the user who creates the business term. The creator of the business term is also its owner by default.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the last user who has updated the business term record. This may be the user who is in the same group as the creator.',
    ...
    KEY                     `business_term_created_by_fk` (`created_by`),
    KEY                     `business_term_last_updated_by_fk` (`last_updated_by`),
    CONSTRAINT `business_term_created_by_fk` FOREIGN KEY (`created_by`) REFERENCES `app_user` (`app_user_id`),
    CONSTRAINT `business_term_last_updated_by_fk` FOREIGN KEY (`last_updated_by`) REFERENCES `app_user` (`app_user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='The Business Term table stores information about the business term, which is usually associated to BIE or CC.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `cc_tag`
(
    `cc_tag_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
...
(
    `cdt_awd_pri_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `cdt_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key pointing to a CDT in the DT table.',
    `cdt_pri_id`     char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the CDT_PRI table.',
...
(
    `cdt_awd_pri_xps_type_map_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `cdt_awd_pri_id`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the CDT_AWD_PRI table.',
    `xbt_id`                      char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the XBT table.',
...
(
    `cdt_pri_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
...
(
    `cdt_ref_spec_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `cdt_id`          char(36) CHARACTER SET ascii NOT NULL,
    `ref_spec_id`     char(36) CHARACTER SET ascii NOT NULL,
...
(
    `cdt_sc_awd_pri_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `cdt_sc_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key pointing to the supplementary component (SC).',
    `cdt_pri_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the CDT_PRI table.',
...
(
    `cdt_sc_awd_pri_xps_type_map_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `cdt_sc_awd_pri_id`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the CDT_SC_AWD_PRI table.',
    `xbt_id`                         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the XBT table.',
    `is_default`                     tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Indicating a default value domain mapping.',
...
(
    `cdt_sc_ref_spec_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `cdt_sc_id`          char(36) CHARACTER SET ascii NOT NULL,
    `ref_spec_id`        char(36) CHARACTER SET ascii NOT NULL,
...
(
    `code_list_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                     char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `agency_id_list_value_id`  char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the AGENCY_ID_LIST_VALUE table. It indicates the organization which maintains the code list.',
    ...
    `namespace_id`             char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the NAMESPACE table.',
    `based_code_list_id`       char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is a foreign key to the CODE_LIST table itself. This identifies the code list on which this code list is based, if any. The derivation may be restriction and/or extension.',
    ...
    `replacement_code_list_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    `created_by`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created the code list.',
    `owner_user_id`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `last_updated_by`          char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It identifies the user who last updated the code list.',
    ...
    `prev_code_list_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_code_list_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
    ...
    KEY                        `code_list_agency_id_list_value_id_fk` (`agency_id_list_value_id`),
    ...
    CONSTRAINT `code_list_agency_id_list_value_id_fk` FOREIGN KEY (`agency_id_list_value_id`) REFERENCES `agency_id_list_value` (`agency_id_list_value_id`),
...
(
    `code_list_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                        char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `code_list_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `based_code_list_manifest_id`       char(36) CHARACTER SET ascii DEFAULT NULL,
    `agency_id_list_value_manifest_id`  char(36) CHARACTER SET ascii DEFAULT NULL,
    `conflict`                          tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `log_id`                            char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A foreign key pointed to a log for the current record.',
    `replacement_code_list_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_code_list_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_code_list_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                                 `code_list_manifest_agency_id_list_value_manifest_id_fk` (`agency_id_list_value_manifest_id`),
    KEY                                 `code_list_manifest_replacement_code_list_manifest_id_fk` (`replacement_code_list_manifest_id`),
    ...
    CONSTRAINT `code_list_manifest_agency_id_list_value_manifest_id_fk` FOREIGN KEY (`agency_id_list_value_manifest_id`) REFERENCES `agency_id_list_value_manifest` (`agency_id_list_value_manifest_id`),
    ...
    CONSTRAINT `code_list_manifest_replacement_code_list_manifest_id_fk` FOREIGN KEY (`replacement_code_list_manifest_id`) REFERENCES `code_list_manifest` (`code_list_manifest_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
    `code_list_value_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                           char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    `code_list_id`                   char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the CODE_LIST table. It indicates the code list this code value belonging to.',
    `based_code_list_value_id`       char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the CODE_LIST_VALUE table itself. This column is used when the CODE_LIST is derived from the based CODE_LIST.',
    ...
    `definition_source`              varchar(100)                 DEFAULT NULL COMMENT 'This is typically a URL identifying the source of the DEFINITION column.',
    `is_deprecated`                  tinyint(1) DEFAULT '0' COMMENT 'Indicates whether the code list value is deprecated and should not be reused (i.e., no new reference to this record should be allowed).',
    `replacement_code_list_value_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    `created_by`                     char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created the code list value.',
    `owner_user_id`                  char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `last_updated_by`                char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It identifies the user who last updated the code list value.',
    `creation_timestamp`             datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP (6) COMMENT 'Timestamp when the code list was created.',
    `last_update_timestamp`          datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP (6) COMMENT 'Timestamp when the code list was last updated.',
    `prev_code_list_value_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_code_list_value_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
...
(
    `code_list_value_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                              char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `code_list_value_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `code_list_manifest_id`                   char(36) CHARACTER SET ascii NOT NULL,
    `based_code_list_value_manifest_id`       char(36) CHARACTER SET ascii DEFAULT NULL,
    `conflict`                                tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `replacement_code_list_value_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_code_list_value_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_code_list_value_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                                       `code_list_value_manifest_replacement_code_list_value_manifest_fk` (`replacement_code_list_value_manifest_id`),
    ...
    CONSTRAINT `code_list_value_manifest_replacement_code_list_value_manifest_fk` FOREIGN KEY (`replacement_code_list_value_manifest_id`) REFERENCES `code_list_value_manifest` (`code_list_value_manifest_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
    `comment_id`            char(36) CHARACTER SET ascii     NOT NULL COMMENT 'Primary, internal database key.',
    `reference`             varchar(100) CHARACTER SET ascii NOT NULL DEFAULT '',
    `comment`               text,
    `is_hidden`             tinyint(1) NOT NULL DEFAULT '0',
    `is_deleted`            tinyint(1) NOT NULL DEFAULT '0',
    `prev_comment_id`       char(36) CHARACTER SET ascii              DEFAULT NULL,
...
(
    `ctx_category_id`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created the context category.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It identifies the user who last updated the context category.',
...
(
    `ctx_scheme_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `ctx_category_id`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'This the foreign key to the CTX_CATEGORY table. It identifies the context category associated with this context scheme.',
    `code_list_id`          char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This is the foreign key to the CODE_LIST table. It identifies the code list associated with this context scheme.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this context scheme.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It identifies the user who last updated the context scheme.',
...
(
    `ctx_scheme_value_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `owner_ctx_scheme_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the CTX_SCHEME table. It identifies the context scheme, to which this scheme value belongs.',
...
CREATE TABLE `dt`
(
    `dt_id`                        char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                         char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `based_dt_id`                  char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key pointing to the DT table itself. This column must be blank when the DT_TYPE is CDT. This column must not be blank when the DT_TYPE is BDT.',
    ...
    `created_by`                   char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this DT.',
    `last_updated_by`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record. \n\nIn the history record, this should always be the user who is editing the entity (perhaps except when the ownership has just been changed).',
    `owner_user_id`                char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    ...
    `is_deprecated`                tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Indicates whether the CC is deprecated and should not be reused (i.e., no new reference to this record should be created).',
    `replacement_dt_id`            char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    `prev_dt_id`                   char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_dt_id`                   char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
    PRIMARY KEY (`dt_id`),
...
(
    `dt_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                 char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `dt_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `based_dt_manifest_id`       char(36) CHARACTER SET ascii DEFAULT NULL,
    `conflict`                   tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `log_id`                     char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A foreign key pointed to a log for the current record.',
    `replacement_dt_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_dt_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_dt_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                          `dt_manifest_replacement_dt_manifest_id_fk` (`replacement_dt_manifest_id`),
    ...
    CONSTRAINT `dt_manifest_replacement_dt_manifest_id_fk` FOREIGN KEY (`replacement_dt_manifest_id`) REFERENCES `dt_manifest` (`dt_manifest_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
    `dt_manifest_id` char(36) CHARACTER SET ascii NOT NULL,
    `cc_tag_id`      char(36) CHARACTER SET ascii NOT NULL,
...
CREATE TABLE `dt_sc`
(
    `dt_sc_id`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `definition_source`     varchar(200)                 DEFAULT NULL COMMENT 'This is typically a URL identifying the source of the DEFINITION column.',
    `owner_dt_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the DT table indicating the data type, to which this supplementary component belongs.',
    ...
    `based_dt_sc_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the DT_SC table itself. This column is used when the SC is derived from the based DT.',
    `default_value`         text COMMENT 'This column specifies the default value constraint. Default and fixed value constraints cannot be used at the same time.',
    `fixed_value`           text COMMENT 'This column captures the fixed value constraint. Default and fixed value constraints cannot be used at the same time.',
    ...
    `replacement_dt_sc_id`  char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement if the record is deprecated.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created the DT_SC.',
    `owner_user_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn''t rollback the ownership.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It identifies the user who last updated the DT_SC.',
    `creation_timestamp`    datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP (6) COMMENT 'Timestamp when the code list was created.',
    `last_update_timestamp` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP (6) COMMENT 'Timestamp when the code list was last updated.',
    `prev_dt_sc_id`         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the previous history record.',
    `next_dt_sc_id`         char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A self-foreign key to indicate the next history record.',
...
(
    `dt_sc_manifest_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`                    char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `dt_sc_id`                      char(36) CHARACTER SET ascii NOT NULL,
    `owner_dt_manifest_id`          char(36) CHARACTER SET ascii NOT NULL,
    `based_dt_sc_manifest_id`       char(36) CHARACTER SET ascii DEFAULT NULL,
    `conflict`                      tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `replacement_dt_sc_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This refers to a replacement manifest if the record is deprecated.',
    `prev_dt_sc_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_dt_sc_manifest_id`        char(36) CHARACTER SET ascii DEFAULT NULL,
    ...
    KEY                             `dt_sc_manifest_based_dt_sc_manifest_id_fk` (`based_dt_sc_manifest_id`),
    KEY                             `dt_sc_manifest_replacement_dt_sc_manifest_id_fk` (`replacement_dt_sc_manifest_id`),
    KEY                             `dt_sc_manifest_prev_dt_sc_manifest_id_fk` (`prev_dt_sc_manifest_id`),
    KEY                             `dt_sc_manifest_next_dt_sc_manifest_id_fk` (`next_dt_sc_manifest_id`),
    CONSTRAINT `dt_sc_manifest_based_dt_sc_manifest_id_fk` FOREIGN KEY (`based_dt_sc_manifest_id`) REFERENCES `dt_sc_manifest` (`dt_sc_manifest_id`),
    CONSTRAINT `dt_sc_manifest_dt_sc_id_fk` FOREIGN KEY (`dt_sc_id`) REFERENCES `dt_sc` (`dt_sc_id`),
    CONSTRAINT `dt_sc_manifest_next_dt_sc_manifest_id_fk` FOREIGN KEY (`next_dt_sc_manifest_id`) REFERENCES `dt_sc_manifest` (`dt_sc_manifest_id`),
    CONSTRAINT `dt_sc_manifest_owner_dt_manifest_id_fk` FOREIGN KEY (`owner_dt_manifest_id`) REFERENCES `dt_manifest` (`dt_manifest_id`),
    CONSTRAINT `dt_sc_manifest_prev_dt_sc_manifest_id_fk` FOREIGN KEY (`prev_dt_sc_manifest_id`) REFERENCES `dt_sc_manifest` (`dt_sc_manifest_id`),
    CONSTRAINT `dt_sc_manifest_release_id_fk` FOREIGN KEY (`release_id`) REFERENCES `release` (`release_id`),
    CONSTRAINT `dt_sc_manifest_replacement_dt_sc_manifest_id_fk` FOREIGN KEY (`replacement_dt_sc_manifest_id`) REFERENCES `dt_sc_manifest` (`dt_sc_manifest_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
    `dt_usage_rule_id`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `assigned_usage_rule_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the USAGE_RULE table indicating the usage rule assigned to a BIE.',
    `target_dt_id`           char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the DT_ID for assigning a usage rule to the corresponding DT content component.',
    `target_dt_sc_id`        char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the DT_SC_ID for assigning a usage rule to the corresponding DT_SC.',
...
(
    `exception_id`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    ...
    `created_by`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who is working on when the exception occurs.',
...
CREATE TABLE `log`
(
    `log_id`                char(36) CHARACTER SET ascii     NOT NULL COMMENT 'Primary, internal database key.',
    ...
    `prev_log_id`           char(36) CHARACTER SET ascii              DEFAULT NULL,
    `next_log_id`           char(36) CHARACTER SET ascii              DEFAULT NULL,
    `created_by`            char(36) CHARACTER SET ascii     NOT NULL,
    `creation_timestamp`    datetime(6) NOT NULL,
    PRIMARY KEY (`log_id`),
    KEY                     `reference` (`reference`),
...
(
    `message_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `sender_id`          char(36) CHARACTER SET ascii NOT NULL COMMENT 'The user who created this record.',
    `recipient_id`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'The user who is a target to possess this record.',
...
CREATE TABLE `module`
(
    `module_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module set.',
    `parent_module_id`      char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'This indicates a parent module id. root module will be NULL.',
    ...
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this MODULE.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record. \n\nIn the history record, this should always be the user who is editing the entity (perhaps except when the ownership has just been changed).',
    `owner_user_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table identifying the user who can update or delete the record.',
    `creation_timestamp`    datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
    `last_update_timestamp` datetime(6) NOT NULL COMMENT 'The timestamp when the record was last updated.',
...
(
    `module_acc_manifest_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_release_id`  char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set release record.',
    `acc_manifest_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the acc manifest record.',
    `module_id`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module.',
    `created_by`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this record.',
    `last_updated_by`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`     datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
    `last_update_timestamp`  datetime(6) NOT NULL COMMENT 'The timestamp when the record was last updated.',
...
(
    `module_agency_id_list_manifest_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_release_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set release record.',
    `agency_id_list_manifest_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the code list manifest record.',
    `module_id`                         char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module.',
    `created_by`                        char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this record.',
    `last_updated_by`                   char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`                datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
...
(
    `module_asccp_manifest_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_release_id`    char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set release record.',
    `asccp_manifest_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the asccp manifest record.',
    `module_id`                char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module.',
    `created_by`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this record.',
    `last_updated_by`          char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`       datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
...
(
    `module_bccp_manifest_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_release_id`   char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set release record.',
    `bccp_manifest_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the bccp manifest record.',
    `module_id`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module.',
    `created_by`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this record.',
    `last_updated_by`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`      datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
...
(
    `module_blob_content_manifest_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_release_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set release record.',
    `blob_content_manifest_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the blob content manifest record.',
    `module_id`                       char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module.',
    `created_by`                      char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this record.',
    `last_updated_by`                 char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`              datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
    ...
    KEY                               `module_blob_content_manifest_last_updated_by_fk` (`last_updated_by`),
    ...
    CONSTRAINT `module_blob_content_manifest_blob_content_manifest_id_fk` FOREIGN KEY (`blob_content_manifest_id`) REFERENCES `blob_content_manifest` (`blob_content_manifest_id`),
...
(
    `module_code_list_manifest_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_release_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set release record.',
    `code_list_manifest_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the code list manifest record.',
    `module_id`                    char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module.',
    `created_by`                   char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this record.',
    `last_updated_by`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`           datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
...
(
    `module_dt_manifest_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_release_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set release record.',
    `dt_manifest_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the dt manifest record.',
    `module_id`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this record.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`    datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
...
(
    `module_set_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this MODULE_SET.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`    datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
    `last_update_timestamp` datetime(6) NOT NULL COMMENT 'The timestamp when the record was last updated.',
...
(
    `module_set_release_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set.',
    `release_id`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    ...
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this MODULE_SET_RELEASE.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`    datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
    `last_update_timestamp` datetime(6) NOT NULL COMMENT 'The timestamp when the record was last updated.',
    PRIMARY KEY (`module_set_release_id`),
    KEY                     `module_set_release_created_by_fk` (`created_by`),
    KEY                     `module_set_release_last_updated_by_fk` (`last_updated_by`),
    KEY                     `module_set_release_release_id_fk` (`release_id`),
    KEY                     `module_set_release_module_set_id_fk` (`module_set_id`),
    CONSTRAINT `module_set_release_created_by_fk` FOREIGN KEY (`created_by`) REFERENCES `app_user` (`app_user_id`),
    CONSTRAINT `module_set_release_last_updated_by_fk` FOREIGN KEY (`last_updated_by`) REFERENCES `app_user` (`app_user_id`),
...
(
    `module_xbt_manifest_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `module_set_release_id`  char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the module set release record.',
    `xbt_manifest_id`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key of the xbt manifest record.',
    `module_id`              char(36) CHARACTER SET ascii NOT NULL COMMENT 'This indicates a module.',
    `created_by`             char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table. It indicates the user who created this record.',
    `last_updated_by`        char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table referring to the last user who updated the record.',
    `creation_timestamp`     datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
    ...
    KEY                      `module_xbt_manifest_xbt_manifest_id_fk` (`xbt_manifest_id`),
    ...
    CONSTRAINT `module_xbt_manifest_xbt_manifest_id_fk` FOREIGN KEY (`xbt_manifest_id`) REFERENCES `xbt_manifest` (`xbt_manifest_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
    `namespace_id`          char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    ...
    `owner_user_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table identifying the user who can update or delete the record.',
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table identifying user who created the namespace.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table identifying the user who last updated the record.',
    `creation_timestamp`    datetime(6) NOT NULL COMMENT 'The timestamp when the record was first created.',
    `last_update_timestamp` datetime(6) NOT NULL COMMENT 'The timestamp when the record was last updated.',
...
(
    `oauth2_app_id`                char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
...
(
    `oauth2_app_scope_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `oauth2_app_id`       char(36) CHARACTER SET ascii NOT NULL,
...
(
    `ref_spec_id` char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
...
(
    `release_id`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
...
(
    `seq_key_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `from_acc_manifest_id` char(36) CHARACTER SET ascii NOT NULL,
    `ascc_manifest_id`     char(36) CHARACTER SET ascii DEFAULT NULL,
    `bcc_manifest_id`      char(36) CHARACTER SET ascii DEFAULT NULL,
    `prev_seq_key_id`      char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_seq_key_id`      char(36) CHARACTER SET ascii DEFAULT NULL,
    PRIMARY KEY (`seq_key_id`),
    KEY                    `seq_key_from_acc_manifest_id_fk` (`from_acc_manifest_id`),
    KEY                    `seq_key_ascc_manifest_id_fk` (`ascc_manifest_id`),
    KEY                    `seq_key_bcc_manifest_id_fk` (`bcc_manifest_id`),
...
(
    `top_level_asbiep_id`   char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `asbiep_id`             char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the ASBIEP table pointing to a record which is a top-level ASBIEP.',
    `owner_user_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table identifying the user who can update or delete the record.',
    ...
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'A foreign key referring to the last user who has updated any related bie records.',
    `release_id`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
...
(
    `usage_rule_id`  char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
...
(
    `usage_rule_expression_id`  char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    ...
    `represented_usage_rule_id` char(36) CHARACTER SET ascii NOT NULL,
...
CREATE TABLE `xbt`
(
    `xbt_id`                char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `guid`                  char(32) CHARACTER SET ascii NOT NULL COMMENT 'A globally unique identifier (GUID).',
    ...
    `subtype_of_xbt_id`     char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'Foreign key to the XBT table itself. It indicates a super type of this XSD built-in type.',
    `schema_definition`     text,
    `revision_doc`          text,
    `state`                 int(11) DEFAULT NULL,
    `created_by`            char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table identifying user who created the xbt.',
    `owner_user_id`         char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table identifying the user who can update or delete the record.',
    `last_updated_by`       char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the APP_USER table identifying the user who last updated the record.',
    `creation_timestamp`    datetime(6) NOT NULL,
    `last_update_timestamp` datetime(6) NOT NULL,
...
(
    `xbt_manifest_id`      char(36) CHARACTER SET ascii NOT NULL COMMENT 'Primary, internal database key.',
    `release_id`           char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the RELEASE table.',
    `xbt_id`               char(36) CHARACTER SET ascii NOT NULL COMMENT 'Foreign key to the XBT table.',
    `conflict`             tinyint(1) NOT NULL DEFAULT '0' COMMENT 'This indicates that there is a conflict between self and relationship.',
    `log_id`               char(36) CHARACTER SET ascii DEFAULT NULL COMMENT 'A foreign key pointed to a log for the current record.',
    `prev_xbt_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL,
    `next_xbt_manifest_id` char(36) CHARACTER SET ascii DEFAULT NULL,
