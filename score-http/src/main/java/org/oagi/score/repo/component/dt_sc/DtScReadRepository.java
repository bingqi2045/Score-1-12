...
public class DtScReadRepository {
    ...
    public DtScRecord getDtScByManifestId(String dtScManifestId) {
    ...
    public List<String> getCdtPrimitivesByManifestId(String dtScManifestId) {
        return dslContext.selectDistinct(CDT_PRI.NAME)
                .from(DT_SC_MANIFEST)
                .join(DT_SC).on(DT_SC_MANIFEST.DT_SC_ID.eq(DT_SC.DT_SC_ID))
                .join(BDT_SC_PRI_RESTRI).on(DT_SC.DT_SC_ID.eq(BDT_SC_PRI_RESTRI.BDT_SC_ID))
