...
public class AsccpCUDRepository {
    ...
    public ReviseAsccpRepositoryResponse reviseAsccp(ReviseAsccpRepositoryRequest request) {
        ...
        AsccpRecord prevAsccpRecord = dslContext.selectFrom(ASCCP)
                .where(ASCCP.ASCCP_ID.eq(prevAsccpManifestRecord.getAsccpId()))
                .fetchOne();
        ...
        ULong targetReleaseId = prevAsccpManifestRecord.getReleaseId();
        if (user.isDeveloper()) {
        ...
        RevisionRecord revisionRecord =
                revisionRepository.insertAsccpRevision(
                        nextAsccpRecord, prevAsccpManifestRecord.getRevisionId(),
                        RevisionAction.Revised,
                        userId, timestamp);

        // creates new asccp manifest for revised record.
        AsccpManifestRecord nextAsccpManifestRecord = prevAsccpManifestRecord.copy();
        ...
        return new ReviseAsccpRepositoryResponse(nextAsccpManifestRecord.getAsccpManifestId().toBigInteger());
    }
