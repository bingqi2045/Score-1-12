...
public class Abie extends TableImpl<AbieRecord> {
    ...
    private static final long serialVersionUID = -349031181;
     ...
     */
    public final TableField<AbieRecord, ULong> ABIE_ID = createField("abie_id", org.jooq.impl.SQLDataType.BIGINTUNSIGNED.nullable(false).identity(true), this, "A internal, primary database key of an ABIE.");
     ...
     */
    public final TableField<AbieRecord, String> GUID = createField("guid", org.jooq.impl.SQLDataType.VARCHAR(41).nullable(false), this, "A globally unique identifier (GUID) of an ABIE. GUID of an ABIE is different from its based ACC. Per OAGIS, a GUID is of the form \"oagis-id-\" followed by a 32 Hex character sequence.");
     ...
     */
    public final TableField<AbieRecord, String> DEFINITION = createField("definition", org.jooq.impl.SQLDataType.CLOB, this, "Definition to override the ACC's definition. If NULL, it means that the definition should be inherited from the based CC.");
     ...
     */
    public final TableField<AbieRecord, ULong> LAST_UPDATED_BY = createField("last_updated_by", org.jooq.impl.SQLDataType.BIGINTUNSIGNED.nullable(false), this, "A foreign key referring to the last user who has updated the ABIE record. This may be the user who is in the same group as the creator.");
     ...
     */
    public final TableField<AbieRecord, Timestamp> CREATION_TIMESTAMP = createField("creation_timestamp", org.jooq.impl.SQLDataType.TIMESTAMP.nullable(false), this, "Timestamp when the ABIE record was first created. ABIEs created as children of another ABIE have the same CREATION_TIMESTAMP.");

    /**
     * The column <code>oagi.abie.last_update_timestamp</code>. The timestamp when the ABIE was last updated.
     */
    public final TableField<AbieRecord, Timestamp> LAST_UPDATE_TIMESTAMP = createField("last_update_timestamp", org.jooq.impl.SQLDataType.TIMESTAMP.nullable(false), this, "The timestamp when the ABIE was last updated.");
     ...
     */
    public final TableField<AbieRecord, Integer> STATE = createField("state", org.jooq.impl.SQLDataType.INTEGER, this, "2 = EDITING, 4 = PUBLISHED. This column is only used with a top-level ABIE, because that is the only entry point for editing. The state value indicates the visibility of the top-level ABIE to users other than the owner. In the user group environment, a logic can apply that other users in the group can see the top-level ABIE only when it is in the 'Published' state.");
     ...
     */
    public final TableField<AbieRecord, String> VERSION = createField("version", org.jooq.impl.SQLDataType.VARCHAR(45), this, "This column hold a version number assigned by the user. This column is only used by the top-level ABIE. No format of version is enforced.");
     ...
     */
    public final TableField<AbieRecord, String> REMARK = createField("remark", org.jooq.impl.SQLDataType.VARCHAR(225), this, "This column allows the user to specify very context-specific usage of the BIE. It is different from the DEFINITION column in that the DEFINITION column is a description conveying the meaning of the associated concept. Remarks may be a very implementation specific instruction or others. For example, BOM BOD, as an ACC, is a generic BOM structure. In a particular context, a BOM ABIE can be a Super BOM. Explanation of the Super BOM concept should be captured in the Definition of the ABIE. A remark about that ABIE may be \"Type of BOM should be recognized in the BOM/typeCode.\"");
     ...
     */
    public final TableField<AbieRecord, String> BIZ_TERM = createField("biz_term", org.jooq.impl.SQLDataType.VARCHAR(225), this, "To indicate what the BIE is called in a particular business context. With this current design, only one business term is allowed per business context.");
    ...
    public List<Index> getIndexes() {
        return Arrays.<Index>asList(Indexes.ABIE_ABIE_BASED_ACC_ID_FK, Indexes.ABIE_ABIE_CLIENT_ID_FK, Indexes.ABIE_ABIE_CREATED_BY_FK, Indexes.ABIE_ABIE_LAST_UPDATED_BY_FK, Indexes.ABIE_ABIE_OWNER_TOP_LEVEL_ABIE_ID_FK, Indexes.ABIE_PRIMARY);
    }
    ...
    public List<ForeignKey<AbieRecord, ?>> getReferences() {
        return Arrays.<ForeignKey<AbieRecord, ?>>asList(Keys.ABIE_BASED_ACC_ID_FK, Keys.ABIE_CREATED_BY_FK, Keys.ABIE_LAST_UPDATED_BY_FK, Keys.ABIE_CLIENT_ID_FK, Keys.ABIE_OWNER_TOP_LEVEL_ABIE_ID_FK);
    }
